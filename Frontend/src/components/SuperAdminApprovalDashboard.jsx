...
const SuperAdminApprovalDashboard = () => {
  ...
  const fetchPendingEvents = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/events/pending-approval', {
        credentials: 'include'
      });
  ...
  const handleApprove = async (eventId) => {
    setProcessing(true);
    try {
      const superAdminId = 1; // This should come from user context/auth
      
      const response = await fetch(`/api/events/approve/${eventId}?superAdminId=${superAdminId}`, {
        method: 'POST',
        credentials: 'include'
      });
      ...
      if (response.ok) {
        ...
        alert(`Failed to approve event: ${error.error}`);
      }
    } catch (error) {
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/events/reject/${selectedEvent.id}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        alert(`Failed to reject event: ${error.error}`);
      }
    } catch (error) {
